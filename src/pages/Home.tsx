  ...
  return (
    <>
      ...
      <section className="px-6 md:px-12 py-24 grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10 max-w-7xl mx-auto">
        <div className="bg-white/5 backdrop-blur-xl border border-white/5 rounded-[32px] p-6 md:p-8 flex items-start gap-5 hover:bg-white/10 transition-colors shadow-2xl">
          ...
          <div>
            <h4 className="font-black uppercase tracking-tight text-lg mb-1 dark:text-white">Custom Arch</h4>
            <p className="text-[10px] font-bold uppercase opacity-50 tracking-widest text-accent">Zero Latency Engineering</p>
          </div>
        </div>
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/5 rounded-[32px] p-6 md:p-8 flex items-start gap-5 hover:bg-white/10 transition-colors shadow-2xl">
          ...
          <div>
            <h4 className="font-black uppercase tracking-tight text-lg mb-1 dark:text-white">E-Commerce</h4>
            <p className="text-[10px] font-bold uppercase opacity-50 tracking-widest text-secondary">Scale Ready Deployment</p>
          </div>
        </div>

        <div className="bg-white/5 backdrop-blur-xl border border-white/5 rounded-[32px] p-6 md:p-8 flex flex-col justify-center items-center text-center hover:bg-white/10 transition-colors shadow-2xl">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="font-black uppercase tracking-tighter text-lg dark:text-white">FBR Certified</span>
          </div>
          <div className="px-5 py-2 bg-black/50 rounded-xl font-mono text-xs text-accent border border-accent/20 shadow-[0_0_15px_rgba(0,240,255,0.1)]">
      ...
      <section className="w-full py-24 relative bg-black/40 border-t border-white/5">
         ...
         <div className="w-full relative overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_5%,black_95%,transparent)]">
            <div className="flex w-max animate-marquee hover:[animation-play-state:paused] gap-6 px-4">
              {doubleTestimonials.map((testimonial, i) => (
                <div 
                  key={i} 
                  className="w-[320px] md:w-[400px] shrink-0 liquid-glass p-6 md:p-8 rounded-3xl border border-white/10 hover:border-accent/40 transition-all duration-300 group flex flex-col"
                >
                  ...
                  <p className="text-gray-300 text-sm font-medium leading-relaxed italic mb-6 flex-grow">"{testimonial.review}"</p>
                  <div className="mt-auto border-t border-white/10 pt-4 flex items-center justify-between">
                    <div>
                      <h5 className="text-white font-black uppercase text-xs tracking-wider">{testimonial.name}</h5>
                      <p className="text-[10px] text-accent uppercase font-bold tracking-widest">{testimonial.role}</p>
                    </div>
                    <div className="w-8 h-8 rounded-full bg-white/10 border border-white/20 flex items-center justify-center text-[10px] font-black text-white mix-blend-overlay">
                      PK
                    </div>
                  </div>
                </div>
        ...
        <div className="container-custom">
           ...
           <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
              {[
                ...
                <div key={idx} className="liquid-glass p-8 rounded-3xl border border-white/5 hover:border-accent/40 transition-all group">
                   <div className="text-4xl font-black text-white/10 group-hover:text-accent/20 mb-4 transition-colors font-mono">{item.step}</div>
                   <h4 className="text-xl font-black uppercase tracking-tight mb-2 dark:text-white">{item.title}</h4>
                   <p className="text-sm font-medium text-gray-400 leading-relaxed uppercase opacity-80">{item.desc}</p>
                </div>
              ))}
           </div>
        </div>
