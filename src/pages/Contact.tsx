        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          <div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <input type="text" value={formData.name} onChange={e=>setFormData({...formData, name: e.target.value})} placeholder="Name" required className="w-full bg-foreground/5 border border-foreground/10 rounded-xl px-5 py-4 focus:border-accent outline-none text-foreground transition-colors" />
              <input type="email" value={formData.email} onChange={e=>setFormData({...formData, email: e.target.value})} placeholder="Email" required className="w-full bg-foreground/5 border border-foreground/10 rounded-xl px-5 py-4 focus:border-accent outline-none text-foreground transition-colors" />
              <input type="tel" value={formData.phone} onChange={e=>setFormData({...formData, phone: e.target.value})} placeholder="Phone" required className="w-full bg-foreground/5 border border-foreground/10 rounded-xl px-5 py-4 focus:border-accent outline-none text-foreground transition-colors" />
              <textarea value={formData.message} onChange={e=>setFormData({...formData, message: e.target.value})} placeholder="Message" required rows={4} className="w-full bg-foreground/5 border border-foreground/10 rounded-xl px-5 py-4 focus:border-accent outline-none text-foreground transition-colors"></textarea>
